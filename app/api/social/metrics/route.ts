...
async function fetchFollowersFromSyndication(username: string) {
  ...
  const first = data?.[0]
  return {
    followers_count: typeof first?.followers_count === "number" ? Math.max(0, first.followers_count) : undefined,
  }
}
...
async function fetchFollowingFromMobile(username: string) {
  ...
  const patterns = [/([\d.,]+)\s*Following/i, /Following\s*([\d.,]+)/i]
...
export async function GET(request: NextRequest) {
  try {
    ...
    const followers_count = followers.status === "fulfilled" ? followers.value.followers_count : undefined
    let following_count = following.status === "fulfilled" ? following.value.following_count : undefined
    ...
    if (
      ...
      following_count = Math.max(10, Math.min(2000, Math.floor(followers_count * 0.4)))
    }

    // If both unknown, provide safe small defaults
    const safeFollowers = Math.max(
      0,
      typeof followers_count === "number" && isFinite(followers_count) ? followers_count : 0,
    )
    const safeFollowing = Math.max(
      0,
      typeof following_count === "number" && isFinite(following_count) ? following_count : 50,
    )
