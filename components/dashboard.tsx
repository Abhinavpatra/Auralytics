...
return 'bg-gray-400 text-black'
  ...
  const isVerified =
    (profile?.isVerified ?? analysisData?.userData?.isVerified) ??
  ...
  const userTier = typeof knownTweetCount === 'number'
    ? (knownTweetCount >= 3000 ? 'POWER USER' : 'STANDARD')
    : undefined
  ...
  <div className="flex justify-between items-center mb-8">
    ...
    <div className="flex items-center gap-4">
      <div className="neobrutalist-card p-4 text-right bg-white dark:bg-black">
        {sessionUser?.name ? (
          <div className="font-black text-lg">{sessionUser.name}</div>
        ) : null}
